...
- 起動時に同期履歴を確認し、消失した「済」リアクションを自動復元
...
サービス起動時に、スクショチャンネルの直近100件と同期チャンネルの履歴を照合します。各家具について最新の有効な同期命令が `1:家具名` なら、Bot 自身の「済」リアクションが消えているメッセージへ再付与します。

空の `0:` / `1:` は無視され、復元処理ではリアクションを削除しません。デプロイ後の初回起動で自動実行されるため、個別の復旧コマンドは不要です。
