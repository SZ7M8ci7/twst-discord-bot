...
- `SHEET_SYNC_INTERVAL_MINUTES`: スプレッドシートとの同期間隔。既定値は `1440` 分（1日）です。
...
サービス起動時にスクショチャンネルを一度索引化し、スプレッドシートの `B:C` を一括取得して照合します。その後は既定で1日ごとにシートを再取得し、`未入力`・`編集中` は「済」を外し、それ以外の状態は「済」を付けます。
